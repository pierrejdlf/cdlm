...
var voronoimap = function(divid) {
	...
	var curMode = 'HF';	
	var pointScale = null; // based on frequentations for each station
	...
	d3.select('#panel_bys').on('mouseover',function(u){
	...
	d3.select('#panel_stationname img').on('click',function(u){
		...
		d3.selectAll('.voropaths').transition().duration(600)
			.attr("fill-opacity",0);
		if(displayMode==1) {
			d3.selectAll('.stations').transition().duration(600)
				.attr("r",minPointSize)
				.attr("opacity",1)
				.attr("fill","gray");
		}
		...
		if(displayMode==3) {
			d3.selectAll('.stations').transition().duration(600)
				.attr("opacity",1)
				.attr("r",function(d){return pointScale(d.trafic);})
				.attr("fill","gray");
		}
		if(displayMode==0) {
			d3.selectAll('.stations').transition().duration(600)
		...
		// rotate M image
		d3.select(this)
			.style("-webkit-transform","rotate(-"+displayMode*90+"deg)")
			.style("-moz-transform","rotate(-"+displayMode*90+"deg)");
	
	});
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	// switch to mode
	f.getWordCount = function(d,word) {
		var cc=0;
		if(d.ngrams.length>0) {
			var listwords = eval('('+d.ngrams+')');
			cc = typeof(listwords[word])=="undefined" ? 0 : listwords[word];
		}
		return cc;	
	}
				...
				wordes[u].forEach(function(thew) {
					clist.append('li')
						...
						.on('click',function(d,i){
							d3.select('#panel_bys').transition().duration(900).style("opacity",0.3);
							var maxOccWord = Math.max.apply(Math,mydata.map(function(y){ return f.getWordCount(y,thew.word); }));
							//console.log("ok"+thew.word);
							var col = "blue";
							var wordscale = d3.scale.linear().domain([0,maxOccWord]).range(["white",col]);
							gvoronoi.selectAll("path")
								.attr("class","voropaths")
								.transition().duration(500)
								.attr("fill-opacity",0.5)
								.attr("fill", function(el,i){
									return wordscale( f.getWordCount(mydata[i],thew.word) );
								});
...
var init_map = function(divid) {
	mapbox.load('minut.map-tno4o0kc', function(data) {
		...
		m.zoom(12).center({lat:48.86,lon:2.32});
...
var initCatCheckboxes = function() {
	var graphcats=[];
	graphcats.push({act:1, col:"#242424", descr:"les 4 types par sexe, toujours visibles", cat:'personne', label:'Types'} );
	...
	graphcats.push({act:0, col:"#274A8C", descr:"rouge, blond, grand, multicolore", cat:'col', label:'Couleurs, formes'} );
	...
	graphcats.push({act:0, col:"#E264F5", descr:"délicieux, ravissant, charmant, ravageur", cat:'bo', label:'Tu es magnifique'} );
	graphcats.push({act:0, col:"#B041F6", descr:"nul, regret, foutu , stupide", cat:'moch', label:'Je suis un con'} );
	graphcats.push({act:0, col:"#89552F", descr:"engager, ouvrir, accrocher, échanger", cat:'action', label:'Actions'} );
	graphcats.push({act:0, col:"#832E2C", descr:"rougir, chagrin, incertitude, amer", cat:'em', label:'Émotions'} );
	graphcats.push({act:0, col:"#718B34", descr:"lourdes, new york, maroc, suisse", cat:'loc', label:'Lieux'} );
	graphcats.push({act:0, col:"#518A32", descr:"matin, midi, soir, siège, strapontin, vitre", cat:'timloc', label:'Espaces'} );
	graphcats.push({act:0, col:"#59368C", descr:"mémoire, soleil, école, erreur, caresse", cat:'sp', label:'Inclassables'} );
