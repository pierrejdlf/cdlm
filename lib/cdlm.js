...
var voronoimap = function(divid) {
	...
	d3.select('#panel_words').on('mouseout',function(e){
		...
		var wantedMin = minWordPointSize;
		if(selWords.length==0) wantedMin = minPointSize;
		f.updatePointsFromWordList(wantedMin,false);
	});
	...
	// selected word list
	f.updatePointsFromWordList = function(wantedMin,smooth) {
		var cumulMax = 0;
		mywords.forEach(function(w){
			if(selWords.indexOf(w.word)!=-1) cumulMax += parseInt(w.max);
		});
		//console.log("cumulMax:"+cumulMax);
		var cumulwordscale = d3.scale.linear().domain([0,cumulMax]).range([wantedMin,maxPointSize]);
		gvoronoi.selectAll(".stations").attr("r", function(d,i){ return cumulwordscale( f.getWordsCount(mydata[i],selWords) ); });
/*
		} else {
			gvoronoi.selectAll(".stations")
				.transition()
				//.attr("fill",function(d,i) { return pointRainbow ? freqPointScale(d.trafic) : pointColor;} )
				.attr("r", function(d,i){ return cumulwordscale( f.getWordsCount(mydata[i],selWords) ); });
		}
*/
	};
				...
				wordes[u].forEach(function(thew) {
					...
					clist.append('li')
						...
						.on('mouseover',function(d,i){
							...
							//var wordscale = d3.scale.linear().domain([0,thew.max]).range([minWordPointSize,maxPointSize]);
							//gvoronoi.selectAll(".stations")
							//	.attr("r", function(el,i){ return wordscale( f.getWordsCount(mydata[i],[thew.word]) ); });
							f.addWordList(thew.word);
							f.updatePointsFromWordList(minWordPointSize,false);
						})
	...
	f.data = function(indata,m) {
		...
		myc = gvoronoi.selectAll('station')
			...
			.attr("class","stations")
			.attr("fill",pointColor)
			.attr("title","") // no need - will be set later on modeswitch //function(d,i){ return f.tooltipStationTitle(d,'HF'); })
...
*/
		...
		f.switchMode('HF');		
/*
		// already made when switching mode
		$(".stations").tooltip({
			'container': 	'body',
			'placement': 	'bottom',
			'trigger': 		'hover',
...
var init_map = function(divid) {
	...
	// disable map tiles, because 10 years passed-by
	// they are not not available anymore
	// i don't really want to update all this
...
var initCatCheckboxes = function() {
	var graphcats=[];
	graphcats.push({act:1, col:"#7E8080", descr:"les 4 types d'annonces, toujours visibles", cat:'personne', label:'Annonces'} );
